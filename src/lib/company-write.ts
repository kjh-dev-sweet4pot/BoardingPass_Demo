...
export async function createCompanyFromBody(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  supabase: SupabaseClient<any>,
  body: Record<string, unknown>,
): Promise<{ company: Company; warning?: string } | { error: string; status: number }> {
  const name = String(body.name || "").trim();
  const login_id = normalizeLoginId(String(body.login_id || ""));
  const password = String(body.password || "");
  if (!name) return { error: "회원사명을 입력하세요.", status: 400 };
  if (!login_id) return { error: "로그인 아이디를 입력하세요.", status: 400 };
  if (!password) return { error: "비밀번호를 입력하세요.", status: 400 };
  ...
  if (crm.error) return { error: crm.error, status: 400 };
  ...
  if (error || !data) {
    ...
    return {
      error:
        status === 409 ? "이미 사용 중인 회원사명 또는 아이디입니다." : msg,
      status,
    };
  }
